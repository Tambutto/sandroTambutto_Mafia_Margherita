...
const usersController = {
    ...
    logout: (req, res) => {
        req.session.destroy((err) => {
            ...
            res.redirect("/login?logout=true"); // Agrega un parámetro para detectar el logout
            // res.redirect("/login");
        })
    },
    ...
    deleteUser: async (req, res) => {
        try {
            ...
            //         error: { status: 403, stack: 'No tienes permisos para eliminar usuarios.' }
            //     });
            // }
            ...
            // **Destruir la sesión si el usuario que se elimina es el que está logueado**
            if (req.session.userLogin && req.session.userLogin.id == id) {
                req.session.destroy((err) => {
